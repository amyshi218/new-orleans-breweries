...
const getCoordinates = (street: string, city: string, state: string) => {
  ...
  let geocodeURL = `https://maps.googleapis.com/maps/api/geocode/json?address=${addStreet(splitStreet)},+${splitCity[0]}+${splitCity[1]},+${state}&key=${process.env.google_api_key}`
